...
The **SHMI** R package provides a complete, reproducible workflow for computing the Soil Health Management Index (SHMI) from standardized Excel workbooks or R‑native example data. SHMI integrates four management sub‑indices:
...
SHMI supports two primary workflows, depending on whether you want to test the package or analyze your own management data.
...
result$indicator_df
```
This example file contains example management data and is already formatted correctly.
It is ideal for:
...
**Important:**
The template is intentionally blank. It contains the required sheets and column structure, but no management data.
You must fill in your crop, disturbance, amendment, and animal records before running `prepare_shmi_inputs()`.
`prepare_shmi_inputs()` performs full validation and will return clear, actionable error messages if:
...
This workflow is the one most users will follow when computing SHMI for their own fields or research datasets
...
- R‑native dataset: [`data(shmi_example)`](https://danielmanter-usda.github.io/SHMI/reference/shmi_example.html)
...
- Disturbance — [`compute_disturbance()`](https://danielmanter-usda.github.io/SHMI/reference/compute_disturbance.html)  
