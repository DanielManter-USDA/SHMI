---
title: "Soil Health Management Index (SHMI)"
pkgdown:
  show_source: false
---
...
The Soil Health Management Index (SHMI) integrates four management indices—**Cover**, **Diversity**, **Inverse Disturbance**, and **Animals**—into a single 0–100 score. Each pillar is computed from harmonized rotation‑scale inputs and then combined using **official national SHMI weights** (locked mode) or user‑specified weights (expert mode).
...
Before any computation, the input list from `prepare_shmi_inputs()` is validated:
...
If validation fails, SHMI computation stops with explicit error messages.
...
- **SHMI = 0** → No cover, no diversity, high disturbance, no organic inputs  
- **SHMI = 100** → Continuous cover, high diversity, no disturbance, frequent organic inputs  
