...
SHMI uses a two‑layer validation system to ensure that all inputs are structurally complete, biologically realistic, and internally consistent before any SHMI pillars are computed. This prevents silent failures, protects against malformed Excel files, and guarantees reproducible SHMI scores.

There are two validators:

1. `validate_excel_input()` — runs before ingestion and checks the raw Excel workbook.
2. `validate_shmi_input()` — runs after ingestion and checks the harmonized SHMI data list.
...
`validate_excel_input()` checks the raw Excel workbook before any processing occurs.
...
All date columns must be valid Excel dates. Text dates or mixed formats cause validation failure.
...
After ingestion and harmonization, `prepare_shmi_inputs()` produces a structured list of SHMI tables. Before computing pillars, `build_shmi()` validates this list using `validate_shmi_input()`.
...
If validation fails at either stage, execution stops with clear, actionable messages.
